...
class LogEntry
    ...
    def initialize(config, entry_string, known_tasks, logger, jstasks, rubtasks, hstasks)
...
class GitLog
  def initialize(config, git_log_source, known_tasks = nil)
    ...
    @git_log.push(LogEntry.new(config, line, known_tasks, @logger, @jstasks, @rubtasks, @hstasks))
    end
    
    k = @logger.length() - 1
    if config.javascript then
        puts "\nJAVASCRIPT TASKS\n\n"
        for i in 0..k
