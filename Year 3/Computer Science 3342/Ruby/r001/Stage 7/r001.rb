...
class LogEntry
    ...
    def initialize(config, entry_string, known_tasks, logger, jstasks, rubtasks, hstasks, rj, rr, rh)
        ...
        if not(entry_string.include?('jkaba'))
            ...
            if(entry_string.include?('reportas') && (entry_string.include?('j001') || entry_string.include?('j100')))
                rj.push(entry_string)
            end
            if(entry_string.include?('reportas') && entry_string.include?('r001'))
                rr.push(entry_string)
            end
            if(entry_string.include?('reportas') && entry_string.include?('h001'))
